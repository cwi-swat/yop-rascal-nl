...
str toSVG(e:turtle(real x, real y, real angle)) 
    = "\<g transform=\"translate(<x * 1.0>, <y * 1.0>) rotate(<(angle-90.0) * 1.0>) matrix(.5 0 0 -.5 -123.25 246.35) \" style=\"transition: opacity 0.3s ease;\" onmouseover=\"this.style.opacity=0\" onmouseout=\"this.style.opacity=1\"\>
      '  \<circle cx=\"250\" cy=\"65\" r=\"50\" fill=\"transparent\" style=\"pointer-events: all;\" /\>
...
str toSVG(e:rectangle(real x, real y, real width, real height)) 
    = "\<rect x=\"<x * 1.0>\" y=\"<y * 1.0>\" width=\"<width * 1.0>\" height=\"<height * 1.0>\" 
      '    <style(e)> /\>";

str toSVG(e:ellipse(real cx, real cy, real rx, real ry)) 
    = "\<ellipse cx=\"<cx * 1.0>\" cy=\"<cy * 1.0>\" rx=\"<rx * 1.0>\" ry=\"<ry * 1.0>\" 
      '    <style(e)> /\>";
    
str toSVG(e:line(real x1, real x2, real y1, real y2)) 
    = "\<line x1=\"<x1 * 1.0>\" x2=\"<x2 * 1.0>\" y1=\"<y1 * 1.0>\" y2=\"<y2 * 1.0>\"
      '    <style(e)> /\>";

str toSVG(e:polyline(lrel[real x, real y] poreals)) 
    = "\<polyline poreals=\"<for (<x,y> <- poreals) {><x * 1.0>,<y * 1.0> <}>\" 
      '    <style(e)> /\>";

str toSVG(e:polygon(lrel[real x, real y] poreals)) 
    = "\<polygon poreals=\"<for (<x,y> <- poreals) {><x * 1.0>,<y * 1.0> <}>\" 
      '    <style(e)> /\>";

str toSVG(group(list[Element] elements)) 
    = toSVG(elements);
...
str toSVG(e:text(real x, real y, real angle, str alinea)) 
    = "\<text x=\"<x * 1.0>\" y=\"<y * 1.0>\"  transform=\"rotate(<angle * 1.0> <x * 1.0> <y * 1.0>) scale(1,-1) translate(<0.0> <-2.0 * y>) \"
      '   <style(e)> \>
      '   <alinea>
      '\</text\>";
